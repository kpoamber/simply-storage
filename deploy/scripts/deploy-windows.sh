#!/usr/bin/env bash
# Deploy script for Innovare Storage on Windows Server (via SSH + Docker Desktop)
# Pulls GHCR image, runs pre-deploy backup, starts services, health checks, rollback on failure
#
# Usage: ./deploy-windows.sh --profile small|medium|large [--image-tag latest] [--skip-backup]
# Environment: POSTGRES_USER, POSTGRES_PASSWORD, POSTGRES_DB, GHCR_TOKEN, GHCR_USER
set -euo pipefail

# --- Configuration ---
DEPLOY_DIR="${DEPLOY_DIR:-C:/innovare-storage}"
...
echo "=== Innovare Storage Deploy (Windows) ==="
