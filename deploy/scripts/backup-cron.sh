...
# --- WAL file rotation (keep last 1 day) ---
WAL_DIR="${BACKUP_DIR}/wal"
WAL_DELETED=0
while IFS= read -r old_wal; do
    rm -f "${old_wal}"
    WAL_DELETED=$((WAL_DELETED + 1))
done < <(find "${WAL_DIR}" -maxdepth 1 -type f -name "0000*" -mtime +1 2>/dev/null)
log "Deleted ${WAL_DELETED} old WAL file(s)"
log "WAL dir size: $(du -sh "${WAL_DIR}" 2>/dev/null | cut -f1 || echo "N/A")"
